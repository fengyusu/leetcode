...
class Solution:
    def openLock(self, deadends: List[str], target: str) -> int:
        ...
        if start_str in deadends or target in deadends:
            return -1
        deadends.append(start_str)
        q = collections.deque()
        q.append(start)

        def update_q(cur_node):
            for j in range(4):
                neighbor_node0 = cur_node.copy()
                neighbor_node0[j] = (neighbor_node0[j] + 1) % 10

                neighbor_node_str = "".join([str(c) for c in neighbor_node0])
                if neighbor_node_str not in deadends:
                    if neighbor_node_str == target:
                        return True
                    q.append(neighbor_node0)
                    deadends.append(neighbor_node_str)

                neighbor_node1 = cur_node.copy()
                neighbor_node1[j] = (neighbor_node1[j] - 1) % 10
                neighbor_node_str = "".join([str(c) for c in neighbor_node1])
                if neighbor_node_str not in deadends:
                    if neighbor_node_str == target:
                        return True
                    q.append(neighbor_node1)
                    deadends.append(neighbor_node_str)
